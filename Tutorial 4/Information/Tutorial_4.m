...
Gamma = 0.04; % try in the range [0.001 0.04 0.1]
...
% Allocate memory
uFB = zeros(2,length(NFTraj),TrialStructure(end));
uFF = uFB;
uExt = uFB;
Force = uFB;
...
for Trial=1:70 % EXTENDED

    if Trial>1
        uFF(:,:,Trial) = Alpha*uFB(:,:,Trial-1) + (1-Gamma)*uFF(:,:,Trial-1);
    end
    ...
    for i=1:size(NFTraj,2)-1
        ...
        % Compute PD control
        uFB(:,i,Trial) = [K*(qDesired(1,i)-q(1,i,Trial))+D*(qDesired(2,i)-q(2,i,Trial));
        ...
        % Update system
        q(:,i+1,Trial) = UpdateSystem(q(:,i,Trial),JointAccel(uFB(:,i,Trial)+uFF(:,i,Trial)+uExt(:,i,Trial),[q(1,i,Trial);q(3,i,Trial)],[q(2,i,Trial);q(4,i,Trial)],H));
    end
...
% TRAJECTORIES AND LATERAL DEVIATION IN FIRST NF

% f2=figure(2); clf(2); set(f2,'name','1(b)','numbertitle','off'); set(gcf,'color','w');
%
% subplot(1,2,1); hold on;
%
% PlotLimits = [-0.02 0.02 -0.00 0.25];
%
% Block = 1; LatDeviation=[];
% for Trial=22:30
%     TempData = eval(strcat('Data1.(strtrim(FieldName(',num2str(Block),',:))).T',num2str(Trial)));
%     plot(TempData(:,1),TempData(:,2));
%
%     [~,MaxVelInd]=max(sqrt(diff(TempData(:,1)).^2+diff(TempData(:,2)).^2));
%     LatDeviation(Trial,1) = TempData(MaxVelInd,1) - TempData(1,1);
%
%     plot(TempData(MaxVelInd,1),TempData(MaxVelInd,2),'.r','markers',20);
% end
%
% plot(0,0.2,'.k','markersize',50);
%
...
%% Simulation plot trajectories on 1 figure
figure(5); close(5); f5=figure(5); set(f5,'name','2(a.extra)','numbertitle','off'); set(gcf,'color','w');
...
axis equal; axis(PlotLimits);
title('NF');
legend('NF before','Channel','VF after','VF before','NF after','location','best');
...
legend('VF before','Channel','VF after','location','best');
